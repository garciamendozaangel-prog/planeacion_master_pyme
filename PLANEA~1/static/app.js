...
let schema = null;
let companyId = localStorage.getItem('pm_company_id') || null;
let state = loadState();
...
function loadState(){
  try {
    const saved = JSON.parse(localStorage.getItem('pm_state'));
...
function bindEvents(){
  document.querySelectorAll('input,textarea,select').forEach(el => el.addEventListener('change', persist));
